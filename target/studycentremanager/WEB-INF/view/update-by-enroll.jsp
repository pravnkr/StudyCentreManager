...
<body>
	...
	<h2>IgnouBadhega - Update an existing Student By Enrollment No</h2>

	<form:form
		action="${pageContext.request.contextPath}/coordinator/update-student/by-enroll/fetch"
		modelAttribute="studentWithEnroll">

		<label>Please Enter the Enrollment no of student:</label>
		<br>
		<form:input path="enrollNo" />
		<input type="submit" value="Fetch A Record">
		<br>
		<form:errors path="enrollNo"></form:errors>

	</form:form>

	<c:if test="${recordNotFetchedByEnroll}">
		<i>Student does not exist with Enroll No: </i>
		<em>${updateCriteriaAttributeVal}.</em>
	</c:if>

	<c:if test="${recordFetchedByEnroll || inputError}">

		<form:form
			action="${pageContext.request.contextPath}/coordinator/update-student/by-enroll/save"
			modelAttribute="fetchedStudentWithEnroll">
			...
			<table>
				...
				<tr>
					...
					<td><form:input path="enrollNo" /><br>
